...
test("visible UI creates and assigns profiles, groups, proxies, VPNs, extensions, and extension groups", async () => {
  ...
  try {
    ...
    await app.invokeError("start_vpn_pool", { poolId: "missing" });
    await app.invokeError("stop_vpn_pool", { poolId: "missing" });
    await app.invokeError("rotate_vpn_pool", { poolId: "missing" });
    await app.invokeError("acquire_vpn_lease", {
      request: {
        pool_id: null,
        country: null,
        providers: [],
    ...
    assert.equal(
      await app.invoke("release_vpn_lease", { leaseId: "missing" }),
      false,
    );
    await app.waitFor(
      async () =>
