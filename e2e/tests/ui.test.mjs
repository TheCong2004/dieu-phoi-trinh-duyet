...
test("all primary navigation buttons and sub-page tabs render and remain interactive", async () => {
  await withApp("ui-navigation", async (app) => {
    ...
    await app.waitForText("VPN Pool");
    await app.clickText("VPN Pool", { roles: ["tab"] });
    await app.waitForText("Create VPN pool");
    assert.match(await app.bodyText(), /Provider accounts|Leases/);
    await dismissSurface(app);
