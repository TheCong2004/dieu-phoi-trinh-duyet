...
test("extensions, extension groups, VPN storage, DNS rules, and event-backed assignments", async () => {
  await withApp("entities-network-extension", async (app) => {
    ...
    assert.deepEqual(await app.invoke("list_vpn_provider_accounts"), []);
    await app.invokeError("add_nordvpn_account", { label: "Nord", token: "" });
    await app.invokeError("add_pia_account", {
      label: "PIA",
      username: "",
      password: "",
    });
    await app.invokeError("validate_vpn_provider_account", {
      accountId: "missing",
    });
    await app.invokeError("delete_vpn_provider_account", {
      accountId: "missing",
    });
    await app.invokeError("list_vpn_provider_countries", {
      provider: "invalid",
    });
    await app.invokeError("import_vpn_provider_configs", {
      provider: "nordvpn",
      request: { account_ids: [], country: null, country_id: null, count: 0 },
    });
    const pool = await app.invoke("create_vpn_pool", {
      request: {
        name: "E2E Pool",
        provider_filter: [],
        country: null,
        ...
        rotation_interval_sec: null,
        rotation_mode: "safe",
        strategy: "round_robin",
        enabled: true,
      },
    });
    assert.equal((await app.invoke("list_vpn_pools")).length, 1);
    assert.deepEqual(await app.invoke("list_vpn_pool_runtimes"), []);
    await app.invoke("update_vpn_pool", {
      poolId: pool.id,
      request: {
        name: "Updated E2E Pool",
        provider_filter: [],
        country: null,
        config_ids: [vpn.id],
        rotation_enabled: false,
    ...
    assert.deepEqual(await app.invoke("list_vpn_leases"), []);
    await app.invoke("delete_vpn_pool", { poolId: pool.id });
