...
impl ExtensionManager {
  ...
  pub fn install_extensions_for_profile(
    ...
    // App-bundled extensions load into every profile with no opt-in, so they
    // come first and apply even when no group is assigned.
    let mut extension_paths = crate::bundled_extensions::loadable_paths();

    let group_id = match &profile.extension_group_id {
      Some(id) => id,
      None => return Ok(extension_paths),
    };

    let group = self.get_group(group_id)?;
    if group.extension_ids.is_empty() {
      return Ok(extension_paths);
    }
