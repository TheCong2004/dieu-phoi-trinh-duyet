...
import { VpnPoolManagement } from "./vpn-pool-management";
...
export function ProxyManagementDialog({
  ...
  const [activeTab, setActiveTab] = useState<"proxies" | "vpns" | "vpnPools">(
    initialTab,
  );
  ...
  const {
    vpnConfigs: vpnOptions,
    vpnUsage,
    isLoading: isLoadingVpns,
  } = useVpnEvents();
  const vpnConfigs = useMemo(
    () => vpnOptions.filter((vpn) => !vpn.is_pool),
    [vpnOptions],
  );
        ...
        <DialogContent className="flex max-h-[85vh] max-w-[min(80rem,calc(100%-4rem))] flex-col">
          ...
          <div className="@container flex min-h-0 w-full flex-1 flex-col">
            <AnimatedTabs
              ...
              onValueChange={(v) =>
                setActiveTab(v as "proxies" | "vpns" | "vpnPools")
              }
            ...
            >
              <div className="flex shrink-0 flex-wrap items-center justify-between gap-2">
                <AnimatedTabsList>
                  ...
                  <AnimatedTabsTrigger value="vpnPools">
                    <span>{t("vpnPool.managementTab")}</span>
                  </AnimatedTabsTrigger>
                </AnimatedTabsList>
              ...
              <AnimatedTabsContent
                ...
                className="mt-4 min-h-0 flex-1 flex-col data-[state=active]:flex"
              >
                <VpnPoolManagement vpnConfigs={vpnConfigs} />
              </AnimatedTabsContent>
            </AnimatedTabs>
          </div>
