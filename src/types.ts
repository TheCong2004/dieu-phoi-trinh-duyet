...
export type VpnProviderKind = "nordvpn" | "piavpn";
export type VpnProviderAccountStatus = "ok" | "expired" | "error";

export interface VpnProviderAccount {
  id: string;
  label: string;
  provider: VpnProviderKind;
  auth_type: "token" | "username_password";
  created_at: number;
  status: VpnProviderAccountStatus;
  connection_cap: number;
  has_credentials: boolean;
}

export interface VpnProviderCountry {
  id?: number;
  code: string;
  name: string;
  server_count: number;
...
export type PoolSelectionStrategy = "round_robin" | "least_recently_used";
export type VpnPoolStatus =
  | "stopped"
  | "starting"
  | "connected"
  | "rotating"
...
export interface VpnPool {
  id: string;
  name: string;
  provider_filter: VpnProviderKind[];
  country?: string;
  config_ids: string[];
  rotation_enabled: boolean;
  rotation_interval_sec?: number;
  rotation_mode: "safe" | "hot";
...
export interface VpnPoolRuntime {
  pool_id: string;
  active_config_id?: string;
  status: VpnPoolStatus;
  exit_ip?: string;
  exit_country?: string;
...
export interface VpnLease {
  ...
  created_at: number;
  expires_at?: number;
  status: "provisioning" | "active" | "releasing" | "failed" | "expired";
}
