...
import type { VpnConfig, VpnPool } from "@/types";
...
export function useVpnEvents() {
  ...
  const loadVpnConfigs = useCallback(async () => {
    try {
      const [configs, pools] = await Promise.all([
        invoke<VpnConfig[]>("list_vpn_configs"),
        invoke<VpnPool[]>("list_vpn_pools"),
      ]);
      const poolOptions: VpnConfig[] = pools
        .filter((pool) => pool.enabled)
        .map((pool) => ({
          id: `pool:${pool.id}`,
          name: i18n.t("vpnPool.profileOption", { name: pool.name }),
          vpn_type: "WireGuard",
          config_data: "",
          created_at: pool.created_at,
          is_pool: true,
        }));
      setVpnConfigs([...configs, ...poolOptions]);
      await loadVpnUsage();
      setError(null);
    } catch (err: unknown) {
  ...
  useEffect(() => {
    ...
    let poolsUnlisten: (() => void) | undefined;
    ...
    const setupListeners = async () => {
      try {
        ...
        poolsUnlisten = await listen("vpn-pools-updated", () => {
          void loadVpnConfigs();
        });
    ...
    return () => {
      ...
      if (poolsUnlisten) poolsUnlisten();
    };
